...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        if self.upscale:
            ...
            x = self.upscaler(x)
        x1 = self.conv1(x)
        x1 = self.bn1(self.relu(x1))
        x2 = self.conv2(x1)
        x2 = self.bn2(self.relu(x2))
...
class Generator(nn.Module):
    def __init__(self, zdim, hdim, im_channels=3, blocks=[2, 2, 2, 2]):
        super(Generator, self).__init__()

        self.initial = nn.Linear(zdim, 4*hdim)
        ...
        for nblocks in blocks:
            for block in range(nblocks):
                if new_dim != current_dim:
                    self.blocks.append(ResBlock(current_dim, new_dim, stride=-2))
                    current_dim = new_dim
    ...
    def forward(self, x):
        ...
        current = current.view(x.shape[0], -1, 2, 2)
...
class ConvDiscriminator(nn.Module):
    def __init__(self, hdim, im_channels=3, blocks=[2, 2, 2, 2]):
    ...
    def forward(self, x):
        current = self.initial(x)
        current = self.initial_bn(self.relu(current))
...
def wgan_objective(model, real, fake, lda=10):
    ...
    for i in range(len(real.shape) - 1):
        alpha.unsqueeze_(-1)
    interps = alpha*real + (1 - alpha)*fake.detach()
    ...
    grad_penalty = (T.norm(grad, p=2, dim=-1) - 1)**2
